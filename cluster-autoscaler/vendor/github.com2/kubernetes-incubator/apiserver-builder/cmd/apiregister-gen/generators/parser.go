	...
	for _, member := range t.Members {
		...
		if !samepkg {
			...
			if len(parts) > 1 {
				...
				if strings.HasPrefix(mSubType.Name.Package, "k8s.io2/api/") {
					...
					case "k8s.io2/apimachinery/pkg/apis/meta/v1":
						// Use versioned types for meta/v1
						uImport = fmt.Sprintf("%s \"%s\"", "metav1", "k8s.io2/apimachinery/pkg/apis/meta/v1")
